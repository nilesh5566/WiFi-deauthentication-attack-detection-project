...
### Method 1 — Static ARP Entry (Best Fix)

Locks the router's real MAC permanently so attackers cannot overwrite it.

**On Windows (Victim PC):**

```cmd
# Add static entry — run as Administrator
netsh interface ip add neighbors "Wi-Fi" "10.238.69.194" "4c:d5:77:be:1c:91"

# Verify it was added
arp -a
# Should show:  10.238.69.194    4c-d5-77-be-1c-91    static

# Remove later if needed
netsh interface ip delete neighbors "Wi-Fi" "10.238.69.194"
```
...
```bash
# Add static entry
sudo arp -s 10.238.69.194 4c:d5:77:be:1c:91

# Verify
arp -a

# Remove later
sudo arp -d 10.238.69.194
```

### Method 2 — Other Prevention Strategies

| Method | How It Helps | Best For |
|--------|-------------|---------|
| Static ARP Entry | Locks MAC — attacker cannot overwrite | Home/small networks |
| Dynamic ARP Inspection (DAI) | Switch validates ARP packets automatically | Enterprise networks |
| HTTPS / VPN | Encrypts traffic — unreadable even if intercepted | All environments |
| Network Monitoring | Continuous detection with Wireshark/IDS | SOC/security teams |
| VLAN Segmentation | Isolates attacker from victim network | Enterprise networks |

### Attack vs Prevention Comparison

| Aspect | Without Prevention | With Prevention |
|--------|-------------------|-----------------|
| ARP Cache | Poisoned by attacker | Locked — cannot change |
| Traffic Flow | Via attacker machine | Direct to router |
| Data Security | All HTTP exposed | Traffic not intercepted |
| Attack Success | ✅ YES — MITM established | ❌ NO — attack fails |
| Detection | Alerts fire | Alerts still fire (attempts visible) |
...
│
├── detector/
│   └── arp_detector.py                ← Python ARP spoof detector (Windows)
│
├── captures/
│   ├── arp_filter.png                 ← Screenshot 1 — arp filter
│   ├── duplicate_address.png          ← Screenshot 2 — duplicate-address-detected
│   └── arp_opcode2.png                ← Screenshot 3 — arp.opcode == 2
│
├── flowchart/
│   └── project_flowchart.png          ← Full project flow diagram
│
├── report/
│   └── Network_Security_Project_Report.docx  ← Full project report
│
└── presentation/
    ├── ARP_Spoofing_Project.pptx      ← Main presentation (13 slides)
    └── Slide_Explanation_Guide.pptx  ← How to present each slide
```
...
All attacks were performed exclusively on:
  ✔ A private network owned by the researcher
  ✔ Devices belonging to the researcher
  ✔ A controlled lab environment with no third parties

Performing ARP spoofing or any MITM attack on networks
...
The authors take no responsibility for misuse of this information.
```
...
| Resource | URL |
|----------|-----|
| Wireshark Documentation | https://wireshark.org/docs |
| Scapy Documentation | https://scapy.readthedocs.io |
| dsniff / arpspoof | https://monkey.org/~dugsong/dsniff |
| Npcap Driver | https://npcap.com |
| Aircrack-ng Suite | https://aircrack-ng.org |
| RFC 826 — ARP Specification | https://tools.ietf.org/html/rfc826 |
...
ARP Spoofing Attack Detection & Prevention  
Platform: Windows + Ubuntu Linux | Tools: Wireshark, Python, Scapy, arpspoof

---

*For educational use only. Always get permission before testing on any network.*
